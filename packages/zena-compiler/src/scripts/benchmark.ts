...
let targets = [
  ...
  // Iteration compile-cost workloads (docs/design/async-iteration.md, "Compile
  // cost of the Step protocol"). Generated by
  // test-files/benchmarks/gen-iteration-benchmarks.mjs; filter with `--filter
  // iter`. These isolate the PER-SITE cost of the three-arm `Step<V>` protocol
  // against the boolean-tuple protocol and plain array iteration. NOTE: they
  // measure a single compile each, so they do NOT reproduce the batch/
  // cross-entry accumulation that dominates whole-suite compile time — that
  // needs the batch measurement described in the design doc.
  {
    name: 'iter-step-distinct-50.zena',
    path: join(benchmarksDir, 'iter-step-distinct-50.zena'),
  },
  {
    name: 'iter-step-distinct-150.zena',
    path: join(benchmarksDir, 'iter-step-distinct-150.zena'),
  },
  {
    name: 'iter-step-same-150.zena',
    path: join(benchmarksDir, 'iter-step-same-150.zena'),
  },
  {
    name: 'iter-bool-150.zena',
    path: join(benchmarksDir, 'iter-bool-150.zena'),
  },
  {
    name: 'iter-array-150.zena',
    path: join(benchmarksDir, 'iter-array-150.zena'),
  },
  {
...
// The TS bootstrap compiler was deleted in the bootstrap retirement
// (docs/design/bootstrap-retirement.md), so its bootstrap-vs-self comparison
// is unobtainable now. Skip it gracefully rather than fail every target, and
// still report the self-hosted compile timings (the number that matters today).
const hasBoot = existsSync(bootstrapCli);
...
if (runCompiler && targets.length > 0) {
  ...
  for (const target of targets) {
    ...
    // --- Benchmark Bootstrap Compiler (only if it still exists) ---
    ...
    const bootMean = hasBoot
      ? bootTimes.reduce((a, b) => a + b, 0) / targetRuns
      : 0;
    ...
    console.log(
      formatRow([
        ...
        hasBoot ? `${bootMean.toFixed(2)} ms` : 'n/a',
        `${selfMean.toFixed(2)} ms`,
        `${selfNodeMean.toFixed(2)} ms`,
        hasBoot ? `${(selfMean / bootMean).toFixed(2)}x` : 'n/a',
        hasBoot ? `${(selfNodeMean / bootMean).toFixed(2)}x` : 'n/a',
      ]),
    );
