...
| `blockmerge.zena`       | CFG cleanup: physically removes blocks unreachable after branch folding (compacts the block list, renumbers successor targets and tryJoin; skips a function whose try-join block would die), and deletes block parameters every incoming edge passes the same value for — uses become the value, the argument leaves every branch record. Runs before and after simplify in the cleanup sequence. |
...
| `harvest.zena`          | After the loop, before any section is emitted: deletes functions unreachable from the exports, the start function, and the vtable installs, following direct calls, tail calls, and `ref_func`; rebinds function indices the way layout does and recomputes the element segment. Globals and types are not pruned yet. |
