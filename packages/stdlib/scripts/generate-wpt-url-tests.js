...
// encoding, so each case's `output` is keyed by encoding. Encoding override is
// an explicit non-goal (DESIGN.md, Scope) — we are always UTF-8 — so only the
...
encEmit(`// Source: web-platform-tests/wpt url/resources/percent-encoding.json`);
encEmit(`// Pinned at commit ${commit}. See tests/url/wpt/README.md.`);
encEmit(`//`);
encEmit(`// Only each case's "utf-8" output is asserted: encoding override is`);
encEmit(`// a non-goal (DESIGN.md, Scope). The fixture drives its inputs`);
encEmit(`// through the query of an https URL, hence the special-query set.`);
...
const encOutPath = join(pkgDir, 'tests', 'url', 'wpt_percent_encoding_test.zena');
